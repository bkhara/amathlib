...
hold on;
plot(x,u,'b*-');
plot(xfine,ufine,'r','LineWidth',1.5);
...
%         figure;
%         hold on;
        plot(x,u,'g.-','LineWidth',1.5);
        plot(xfine,ufine,'k','LineWidth',1.5);
        legend('computed\_initial','true\_initial','computed\_final','true\_final');
        title(sprintf('t = %9.5e  after %4i time steps with %5i grid points',...
            tnp,n,m+2));
        grid on;
        
        error = max(abs(u-utrue(x,tnp)));
        %error = sqrt((h)*sum((u-utrue(x,tnp)).^2));
